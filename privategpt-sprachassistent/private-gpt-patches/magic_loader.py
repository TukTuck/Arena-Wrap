...
def load_lib():
  # PATCHED for Windows: loading the MSYS/Git-Bash libmagic DLL hangs in a
  # native ctypes call. Fail fast instead so callers can fall back.
  if sys.platform in ('win32', 'cygwin'):
    raise ImportError('libmagic unavailable on Windows (patched to avoid hanging MSYS DLL)')
